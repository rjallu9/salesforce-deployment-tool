				...
				switch (message.command) {
					...
					case 'loadTypesComponents':
						...
						if (fs.existsSync(metdataPath)) {
							...
							.then((data:any) => {
								...
								Array.from(data.sobjects.values()).flat().forEach((name:any) => {
								...
								fs.writeFile(context.globalStorageUri.fsPath+"/"+sourceOrg.orgId+"/metadata.json", JSON.stringify(Array.from(metadata), null, 2), 'utf8', (err:any) => {
									if (err) {
					...
					case 'updateSnapshot':
						if(message.data) {
							...
							fs.writeFile(context.globalStorageUri.fsPath+"/snapshots.json", JSON.stringify(message.data, null, 2), 'utf8', (err:any) => {
								if (err) {
