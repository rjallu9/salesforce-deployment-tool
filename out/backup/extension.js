        ...
        panel.webview.onDidReceiveMessage((message) => {
            switch (message.command) {
                case 'requestUserData':
                    vscode.window.showInformationMessage('Connecting to the Salesforce Org');
                    getUserData().then((result) => {
                        let jsonData = result;
                        userInfo.accessToken = jsonData.accessToken;
                        userInfo.instanceUrl = jsonData.instanceUrl;
                        userInfo.username = jsonData.username;
                        userInfo.orgId = jsonData.orgId;
                        panel.webview.postMessage({ command: 'userDetails', userInfo: jsonData });
                    }).catch((error) => {
                        vscode.window.showErrorMessage(`Unable to connect to the Salesforce Org`);
                    });
                    break;
                case 'loadTypes':
                    getTypes(userInfo.accessToken, userInfo.instanceUrl, favorites_json_1.default)
                        .then((data) => {
                ...
                case 'loadComponents':
                    if (message.data) {
                        vscode.window.showInformationMessage(`Loading: ${JSON.stringify(message.data)}`);
                        getComponents(userInfo.accessToken, userInfo.instanceUrl, message.data)
                            .then((data) => {
                            panel.webview.postMessage({ command: 'components', components: data, type: message.data });
                        })
                            .catch((error) => {
                            vscode.window.showErrorMessage(`Error: ${JSON.stringify(error)}`);
                        });
                ...
                case 'updateFavorites':
                    ...
                    break;
                case 'getAuthOrgs':
                    getAuthOrgs(userInfo.orgId).then((result) => {
                        orgsList = result;
...
function getUserData() {
    return new Promise((resolve, reject) => {
        //sf org list --json
        /*exec('sf org display user --json', (error:any, stdout:any, stderr:any) => {
            if (error) {
                reject(`Error: ${error}`);
            } else {
                try {
        ...
        resolve({ "accessToken": "00D3t000004pIgV!AQgAQEO69Rytvu_P0b44O3JTMuYd9oTvN3FwKul9QdoScZw6NjwKZUi1iXDUxmnAFZkZmn1CDcMoedcT3Pfpk5RGDUroYBNr",
            "id": "0053t00000A3GlfAAF", "instanceUrl": "https://ice7-dev-ed.my.salesforce.com", "loginUrl": "https://login.salesforce.com",
            "orgId": "00D3t000004pIgVEAU", "profileName": "System Administrator", "username": "ramu.jallu@gmail.com", "alias": "ICE" });
    });
}
function getAuthOrgs(defaultOrgId) {
    return new Promise((resolve, reject) => {
        /*exec('sf org list auth --json', (error:any, stdout:any, stderr:any) => {
            if (error) {
                reject(`Error: ${error}`);
            } else {
                try {
                    ...
                    data.forEach((org:any) => {
                        if(org.orgId !== defaultOrgId && org.isExpired.toString() !== 'true') {
                            orgList.push({
